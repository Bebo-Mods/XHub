...
function UILib.CreateWindow(libName, version, hideonlaunch)
	...
	key1.Text = "LeftShift"
	key1.TextColor3 = Color3.fromRGB(0, 155, 255)
	...
	if hideonlaunch and hideonlaunch == true then LuxtLib.Enabled = false end
	...
	hubLogo.BackgroundColor3 = Color3.fromRGB(0, 155, 255)
	hubLogo.Position = UDim2.new(0.0567928664, 0, 0.0243411884, 0)
	hubLogo.Size = UDim2.new(0, 30, 0, 30)
	hubLogo.ZIndex = 2
	hubLogo.Image = "http://www.roblox.com/asset/?id=9838245330"
	...
	--hubName.TextColor3 = Color3.fromRGB(0, 155, 255)
	hubName.TextColor3 = Color3.fromRGB(0,155,255)
	...
	usename.TextColor3 = Color3.fromRGB(3, 188, 255)
	...
	wave.Size = UDim2.new(0.97865659, 0, 1, 0)
	--wave.Image = "rbxassetid://11941099481"
	wave.Image = "http://www.roblox.com/asset/?id=11941257096"
	...
	function TabHandling:ToggleUi()
		LuxtLib.Enabled = not LuxtLib.Enabled
	end

	function TabHandling:Tab(tabText, tabId, IsLocked)
		...
		tabBtn.Text = tabText
		--tabBtn.TextColor3 = Color3.fromRGB(0, 155, 255)
		tabBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
		tabBtn.TextSize = 14.000
		tabBtn.TextXAlignment = Enum.TextXAlignment.Left
		tabBtn.TextStrokeColor3 = Color3.fromRGB(0,155,255)
		...
		tabLogo.ImageColor3 = Color3.fromRGB(0, 155, 255)
		--
		...
		local function UpdateSize()
			...
			local SizeY = cS.Y
			for i,v in pairs(SectionListsX) do
				if v.Dropped then
					SizeY=SizeY+v.Sec.AbsoluteContentSize.Y
				end
			end
			...
			game.TweenService:Create(
				...
				{
					CanvasSize = UDim2.new(0, cS.X, 0, SizeY)
				}
			):Play()
		end
		UpdateSize()
			...
			function()
				if IsLocked then
					game.TweenService:Create(
						...
						{
							TextColor3 = Color3.fromRGB(234, 19, 48),
							Rotation = 5,
							Position = UDim2.new(0.285534033, 0, 0, 0)
						}
					):Play() 
					task.wait(.05)
					game.TweenService:Create(
				...
				game.TweenService:Create(
					...
					TweenInfo.new(0.18, Enum.EasingStyle.Quint, Enum.EasingDirection.In),
					{
						ImageColor3 = Color3.fromRGB(0, 155, 255)
					}
				):Play()
				game.TweenService:Create(
					tabBtn,
		...
		function sectionHandling:Label(labelInfo, customcolor, CustomAlign)
			...
			UIListLayout.HorizontalAlignment = (CustomAlign) or Enum.HorizontalAlignment.Center
			UIListLayout.SortOrder = Enum.SortOrder.LayoutOrder
			...
			TextLabel.TextColor3 = (customcolor) or Color3.fromRGB(255, 255, 255)
		...
		function sectionHandling:DeletableLabel(labelInfo, customcolor, CustomAlign)
			local TextLabelFrame = Instance.new("Frame")
			local UIListLayout = Instance.new("UIListLayout")
			local TextLabel = Instance.new("TextLabel")
			local UICorner = Instance.new("UICorner")
		...
		function sectionHandling:Section(sectionText, conditionax)
			conditionax = conditionax or {S=true, cb=function() end}
			...
			--sectionName.TextColor3 = Color3.fromRGB(0, 155, 255)
			...
			sectionExpannd.ImageColor3 = Color3.fromRGB(0, 155, 255)
			sectionExpannd.ImageRectOffset = Vector2.new(564, 284)
			sectionExpannd.ImageRectSize = Vector2.new(36, 36)

			local SectionLists = {}

			--local ButId = "Button_"..math.random(1000,9999)*math.random(1000,9999)		
			--SectionLists[SecID].Items[ButId] = ButtonFrame

			local function UpS()
				local cS = sectionInnerList.AbsoluteContentSize.Y+5
				local SizeY = cS

				-- Calculate True Size, Ik It's Ugly.

				for i,v in pairs(SectionLists) do
					if v.Dropped then
						for _,q in pairs(v.Items) do
							--print(SizeY, q.Size, q.Size.Y)
							SizeY = SizeY + q.Size.Y.Offset
						end
					end
				end


				--return UDim2.new(1, 0, 0, sectionInnerList.AbsoluteContentSize.Y + 5)
			...
			sectionExpannd.MouseButton1Click:Connect(
				function()
					if conditionax and conditionax.S == false then conditionax.cb(); return end
					if isDropped then
						isDropped = false
			...
			function ItemHandling:ToggleLock(lock)
				local Originaltext = sectionText
				if lock == 1 then sectionName.Text = "✔️ "..Originaltext end
				if lock == 2 then sectionName.Text = "❌ "..Originaltext end
				if lock == 3 then sectionName.Text = ""..Originaltext end
			end

			function ItemHandling:UpdateSize()
				sectionFrame:TweenSize(
			...
			function ItemHandling:Button(btnText, callback)
				...
				local StringLen = #string.split(btnText, "")
				local ExtraLines = 0				
				repeat
					if (StringLen-50)>=0 then
						StringLen = StringLen - 60
				...
				ButtonFrame.Size = UDim2.new(0, 365, 0, 36+(18*ExtraLines))

				TextButton.Parent = ButtonFrame
				TextButton.BackgroundColor3 = Color3.fromRGB(18, 18, 18)
				TextButton.Size = UDim2.new(0, 365, 0, 36+(18*ExtraLines))
				TextButton.ZIndex = 2
				...
				local ButtonHandling = {}
				function ButtonHandling:Destroy()
					ButtonFrame:Destroy()
				end

				function ButtonHandling:UpdateText(Text)
					local Originaltext = btnText
			...
			function ItemHandling:Toggle(toggInfo, enabledByDefault, callback)
				...
				if enabledByDefault then
					checkBtn.Parent.toggleInfo.TextColor3 = Color3.fromRGB(0, 155, 255)
					checkBtn.ImageColor3 = Color3.fromRGB(0, 155, 255)
				...
				local ToggleHandling = {}
				function ToggleHandling:ToggleLock(lock)
					local Originaltext = toggInfo
					if lock == 1 then toggleInfo.Text = "✔️ "..Originaltext end
					if lock == 2 then toggleInfo.Text = "❌ "..Originaltext end
					if lock == 3 then toggleInfo.Text = ""..Originaltext end
				end

				function ToggleHandling:UpdateText(Text)
					local Originaltext = toggInfo
					toggleInfo.Text = Text
				end

				function ToggleHandling:SetPrice(Price, Image)
					if (not ToggleFrame:FindFirstChild("pricingInfo")) then
						local pricingInfo = Instance.new("TextLabel")
						pricingInfo.Name = "pricingInfo"
						pricingInfo.Parent = ToggleFrame
						pricingInfo.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
						pricingInfo.BackgroundTransparency = 1
						pricingInfo.Position = UDim2.new(0.875, -50,0.15, 0)
						pricingInfo.Size = UDim2.new(0.1, 20, 0.75, 0)
						pricingInfo.ZIndex = 2
						pricingInfo.Font = Enum.Font.FredokaOne
						pricingInfo.Text = Price
						pricingInfo.TextColor3 = Color3.fromRGB(255,255,255)
						pricingInfo.TextSize = 18.000
						pricingInfo.TextXAlignment = Enum.TextXAlignment.Center
						--local x = Instance.new("UICorner", pricingInfo)
						--x.CornerRadius = UDim.new(.25,0)

						local Y = Instance.new("ImageLabel", pricingInfo)
						Y.Image = Image
						Y.Position = UDim2.new(1.05, 0,-0.075, 0)
						Y.Size = UDim2.new(0.5, 0,0, 100)
						Y.BackgroundTransparency = 1

						local Q = Instance.new("UIAspectRatioConstraint", Y)
					end
				end

				function ToggleHandling:ToggleOff()
					on = false
					checkBtn.Parent.toggleInfo.TextColor3 = Color3.fromRGB(0, 155, 255)
					checkBtn.ImageColor3 = Color3.fromRGB(0, 155, 255)
					checkBtn.ImageRectOffset = Vector2.new(4, 836)
				...
				function ToggleHandling:ToggleOn()
					on = true
					checkBtn.Parent.toggleInfo.TextColor3 = Color3.fromRGB(97, 97, 97)
					checkBtn.ImageColor3 = Color3.fromRGB(97, 97, 97)
					checkBtn.ImageRectOffset = Vector2.new(940, 784)
			...
			function ItemHandling:KeyBind(keyInfo, first, callback)
				...
				key.Text = oldKey
				key.TextColor3 = Color3.fromRGB(0, 155, 255)
				key.TextSize = 14.000
			...
			function ItemHandling:TextBox(infbix, textPlace, callback)
				...
				--TextBox.BackgroundColor3 = Color3.fromRGB(0, 155, 255)
			...
			function ItemHandling:Slider(slidInfo, minvalue, maxvalue, callback)
				...
				dragSlider.BackgroundColor3 = Color3.fromRGB(0, 155, 255)
						...
						moveconnection =
							mouse.Move:Connect(
								function()
									...
									dragPrecent.Text = tostring(Value).."/"..tostring(maxvalue)
									Value =
									math.floor(
										(((tonumber(maxvalue) - tonumber(minvalue)) / 150) * dragSlider.AbsoluteSize.X) +
										tonumber(minvalue)
			...
			function ItemHandling:Label(labelInfo, customcolor, CustomAlign)
				local TextLabelFrame = Instance.new("Frame")
				local UIListLayout = Instance.new("UIListLayout")
				local TextLabel = Instance.new("TextLabel")
				local UICorner = Instance.new("UICorner")
			...
			function ItemHandling:DeletableLabel(labelInfo, customcolor, CustomAlign)
				local TextLabelFrame = Instance.new("Frame")
				local UIListLayout = Instance.new("UIListLayout")
				local TextLabel = Instance.new("TextLabel")
				local UICorner = Instance.new("UICorner")
			...
			function ItemHandling:Credit(creditWho)
				...
				local CreditHandling = {}
				function CreditHandling:Update(Text)
					TextLabel.Text = Text
				end
				return CreditHandling
			end
			...
			function ItemHandling:DropDown(dropInfo, list, callback)
				...
				expand_more.ImageColor3 = Color3.fromRGB(0, 155, 255)
				...
				dropdownItem1.TextColor3 = Color3.fromRGB(0, 155, 255)
			...
			function ItemHandling:Section(sectionTextX, conditional)
				conditional = conditional or {S=true, cb=function() end}
				local SecID = "Sec_"..math.random(1000,9999)*math.random(1000,9999)
				local InsectionFrame = Instance.new("TextButton")
				local InMainCorner = Instance.new("UICorner")
				local InmainSectionHead = Instance.new("Frame")
				local InsectionName = Instance.new("TextLabel")
				local InsectionExpannd = Instance.new("ImageButton")

				local InsectionInnerList = Instance.new("UIListLayout")

				InsectionInnerList.Name = "sectionInnerList"
				InsectionInnerList.Parent = InsectionFrame
				InsectionInnerList.HorizontalAlignment = Enum.HorizontalAlignment.Center
				InsectionInnerList.SortOrder = Enum.SortOrder.LayoutOrder
				InsectionInnerList.Padding = UDim.new(0, 3)

				--
				sectionTextX = sectionTextX or "Section"
				local isDroppedX = false
				SectionLists[SecID] = {
					Dropped = isDroppedX,
					Items = {}
				}
				...
				InsectionFrame.Name = "sectionFrame"
				InsectionFrame.Text = ""
				InsectionFrame.Parent = sectionFrame
				InsectionFrame.BackgroundColor3 = Color3.fromRGB(21, 21, 21)
				InsectionFrame.Position = UDim2.new(0, 0, 7.08064434e-08, 0)
				InsectionFrame.Size = UDim2.new(1, 0, 0, 36)
				InsectionFrame.ZIndex = 2
				InsectionFrame.ClipsDescendants = true
				InsectionFrame.AutoButtonColor = false

				InMainCorner.CornerRadius = UDim.new(0, 5)
				InMainCorner.Name = "MainCorner"
				InMainCorner.Parent = InsectionFrame

				InmainSectionHead.Name = "mainSectionHead"
				InmainSectionHead.Parent = InsectionFrame
				InmainSectionHead.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
				InmainSectionHead.BackgroundTransparency = 1.000
				InmainSectionHead.BorderSizePixel = 0
				InmainSectionHead.Size = UDim2.new(0, 381, 0, 36)

				InsectionName.Name = "sectionName"
				InsectionName.Parent = InmainSectionHead
				InsectionName.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
				InsectionName.BackgroundTransparency = 1.000
				InsectionName.Position = UDim2.new(0.0236220472, 0, 0, 0)
				InsectionName.Size = UDim2.new(0, 302, 0, 36)
				InsectionName.Font = Enum.Font.GothamSemibold
				InsectionName.Text = sectionTextX
				--sectionName.TextColor3 = Color3.fromRGB(0, 155, 255)
				InsectionName.TextColor3 = Color3.fromRGB(255, 255, 255)
				InsectionName.TextSize = 14.000
				InsectionName.TextXAlignment = Enum.TextXAlignment.Left

				InsectionExpannd.Name = "sectionExpannd"
				InsectionExpannd.Parent = InmainSectionHead
				InsectionExpannd.BackgroundTransparency = 1.000
				InsectionExpannd.Position = UDim2.new(0.91863519, 0, 0.138888896, 0)
				InsectionExpannd.Size = UDim2.new(0, 25, 0, 25)
				InsectionExpannd.ZIndex = 2
				InsectionExpannd.Image = "rbxassetid://3926305904"
				InsectionExpannd.ImageColor3 = Color3.fromRGB(0, 155, 255)
				InsectionExpannd.ImageRectOffset = Vector2.new(564, 284)
				InsectionExpannd.ImageRectSize = Vector2.new(36, 36)

				InsectionExpannd.MouseButton1Click:Connect(
					function()
						if conditional and conditional.S == false then conditional.cb(); return end
						if isDroppedX then
							isDroppedX = false
							SectionLists[SecID].Dropped = false;SectionListsX[SecID].Dropped = false
							InsectionFrame:TweenSize(UDim2.new(1, 0, 0, 36), "In", "Quint", 0.10)
							game.TweenService:Create(
								InsectionExpannd,
								TweenInfo.new(0.10, Enum.EasingStyle.Quad, Enum.EasingDirection.In),
								{
							...
							isDroppedX = true
							SectionLists[SecID].Dropped = true;SectionListsX[SecID].Dropped = true
							InsectionFrame:TweenSize(
								UDim2.new(1, 0, 0, InsectionInnerList.AbsoluteContentSize.Y + 5),
								"In",
								"Quint",
								0.10
							)
				...
				InsectionFrame.MouseButton1Click:Connect(
					function()
						if conditional and conditional.S == false then conditional.cb(); return end
						if isDroppedX then
							isDroppedX = false
				...
				local InItemHandling = {}

				function InItemHandling:Toggle(toggInfo, enabledByDefault, callback)
					local ToggleFrame = Instance.new("Frame")
					local toggleFrame = Instance.new("TextButton")
					local UICorner = Instance.new("UICorner")
					local checkBtn = Instance.new("ImageButton")
					...
					ToggleFrame.Name = "ToggleFrame"
					ToggleFrame.Parent = InsectionFrame
					ToggleFrame.BackgroundColor3 = Color3.fromRGB(18, 18, 18)
					ToggleFrame.BackgroundTransparency = 1.000
					ToggleFrame.Size = UDim2.new(0, 365, 0, 36)
				...
				function InItemHandling:Button(btnText, callback)			
					...
					ButtonFrame.Name = "ButtonFrame"
					ButtonFrame.Parent = InsectionFrame
					ButtonFrame.BackgroundColor3 = Color3.fromRGB(18, 18, 18)
					ButtonFrame.BackgroundTransparency = 1.000
					ButtonFrame.Size = UDim2.new(0, 365, 0, 36+(18*ExtraLines))
